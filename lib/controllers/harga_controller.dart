...
import 'package:admin/models/model_harga.dart';
import 'package:fluttertoast/fluttertoast.dart';
import 'login_controller.dart';

class HargaController with ChangeNotifier {
  List data_HargaList = [];
  List DataDetailList = [];
  List DataImage = [];
  bool proses = false;
  ...
  model_Harga m_Harga = model_Harga();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_HargaList =
        await m_Harga.data_Harga_paginate(cari, FLAG, DR, SUB, offset, limit);
    var count = await m_Harga.count_Harga_paginate(cari, FLAG, DR, SUB);
  ...
  Future<void> Detail_Harga(String NO_BUKTI, String paramtabel) async {
    DataDetailList = [];
    WidgetsBinding.instance?.addPostFrameCallback((_) {
      dataDetailNotif.clear();
    });
    DataDetailList = await m_Harga.select_Harga_detail(NO_BUKTI, paramtabel);
  ...
  Future update_Harga(String NO_BUKTI) async {
    Map obj = new Map();
    obj['NO_BUKTI'] = NO_BUKTI;
    obj['COBA_VAL'] = 1;
    obj['COBA_USRVAL'] = LoginController.nama_staff;
    obj['COBA_TGLVAL'] = DateTime.now();
    await m_Harga.update_Harga(obj, DataDetailList.length);
  ...
  Future Data_Image(String NO_BUKTI, String paramtabel) async {
    DataDetailList = [];
    DataImage = await m_Harga.select_Image(NO_BUKTI, paramtabel);
    notifyListeners();
  }
}
