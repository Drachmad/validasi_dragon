...
import 'package:admin/models/model_pp.dart';
import 'package:fluttertoast/fluttertoast.dart';
import 'login_controller.dart';

class ValPpController with ChangeNotifier {
  List data_ValPpList = [];
  List DataDetailList = [];
  List DataImage = [];
  bool proses = false;
  ...
  model_ValPp m_ValPp = model_ValPp();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_ValPpList =
        await m_ValPp.data_ValPp_paginate(cari, FLAG, DR, SUB, offset, limit);
    var count = await m_ValPp.count_ValPp_paginate(cari, FLAG, DR, SUB);
  ...
  Future<void> Detail_ValPp(String NO_BUKTI, String paramtabel) async {
    DataDetailList = [];
    WidgetsBinding.instance?.addPostFrameCallback((_) {
      dataDetailNotif.clear();
    });
    DataDetailList = await m_ValPp.select_ValPp_detail(NO_BUKTI, paramtabel);
  ...
  Future update_ValPp(String NO_BUKTI) async {
    Map obj = new Map();
    obj['NO_BUKTI'] = NO_BUKTI;
    obj['COBA_VAL'] = 1;
    obj['COBA_USRVAL'] = LoginController.nama_staff;
    obj['COBA_TGLVAL'] = DateTime.now();
    await m_ValPp.update_ValPp(obj, DataDetailList.length);
  ...
  Future Data_Image(String NO_BUKTI, String paramtabel) async {
    DataDetailList = [];
    DataImage = await m_ValPp.select_Image(NO_BUKTI, paramtabel);
    notifyListeners();
  }
}
